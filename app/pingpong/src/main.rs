...
async fn main() {
    ...
    let mut address: String = String::from("0.0.0.0:3033");

    match env::var("PING_PORT") {
        Ok(val) => {
            address = address.replace("3033", &val);
        }
        Err(_e) => println!("Environment variable PORT not defined. Using default port 3033"),
    }
